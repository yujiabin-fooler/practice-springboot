...
public class WordVectorModel extends AbstractVectorModel<String>
{
    ...
    {
        this(modelFileName, new TreeMap<String, java.util.Vector>());
    }
     ...
     * @throws IOException 加载错误
     */
    public WordVectorModel(String modelFileName, Map<String, java.util.Vector> storage) throws IOException
    {
        super(loadVectorMap(modelFileName, storage));
    }

    private static Map<String, java.util.Vector> loadVectorMap(String modelFileName, Map<String, java.util.Vector> storage) throws IOException
    {
        VectorsReader reader = new VectorsReader(modelFileName);
        reader.readVectorFile();
        for (int i = 0; i < reader.vocab.length; i++)
        {
            storage.put(reader.vocab[i], new java.util.Vector(reader.matrix[i]));
        }
        return storage;
    }
    ...
    {
        java.util.Vector a = storage.get(A);
        java.util.Vector b = storage.get(B);
        java.util.Vector c = storage.get(C);
